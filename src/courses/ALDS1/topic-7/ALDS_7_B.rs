...
fn main() {
    ...
    for i in 0..n {
        ...
        i,
        if let Some(r) = nodes[i].parent { r as i32 } else { -1 },
        if let Some(v) = nodes[i].sibling { v as i32 } else { -1 },
        nodes[i].degree,
        nodes[i].depth,
        nodes[i].height,
        if nodes[i].parent.is_none() { "root" } else if nodes[i].degree > 0 { "internal node" } else { "leaf" });
    }
}
